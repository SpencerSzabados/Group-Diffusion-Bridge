#!/bin/bash
#SBATCH --job-name=sample_ddbm_vae
#SBATCH --nodes=1
...
source ./args.sh $DATASET_NAME vae_fives_patches $PRED vp
...
NGPU=1
CHURN_STEP_RATIO=0
MODEL_PATH=/u6/sszabado/checkpoints/ddbm/h2e_rot90_fives_patches__3ich_128ch_3b_vp/model_060000.pt

NCCL_P2P_LEVEL=NVL mpiexec --allow-run-as-root -n $NGPU python image_sample.py --work_dir $WORK_DIR --exp=$EXP \
--num_samples 4 --churn_step_ratio $CHURN_STEP_RATIO --steps $N --sampler $GEN_SAMPLER \
--model_path $MODEL_PATH --work_dir=$WORK_DIR --exp=$EXP --data_dir=$DATA_DIR --dataset=$DATASET \
